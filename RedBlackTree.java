...
public class RedBlackTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
	...
	/**
	 * Ensures that the red black tree has no property violations. Will fix every
	 * violation that appears by rotating and recoloring. This method should be
	 * called every time a new node is added or a node that was previously black is
	 * changed to red(The latter is called recursively).
	 * 
	 * @param node The new node that is added or previously black node that is changed to red
	 */
	private void enforceRBTreePropertiesAfterInsert(Node<T> node) {
		if (node == null || node == root || node.parent.isBlack) {
			return;
		}

		if (node.parent.isLeftChild()) {// Parent = leftChild
			if (node.parent.parent.rightChild == null || node.parent.parent.rightChild.isBlack) {
				// Black sibling case
	...
	public static void main(String[] arg) {
		RedBlackTree<Integer> tree = new RedBlackTree<Integer>();
		 ...
		 tree.insert(27);
		 System.out.println(tree.root); 
		 tree.test1(); tree.test2(); tree.test3();
		 tree.test4();
